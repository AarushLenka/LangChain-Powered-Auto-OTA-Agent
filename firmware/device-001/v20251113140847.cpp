...
// Description: Enhanced system health check with temperature threshold management and power optimization.
#include <Arduino.h>
...
const int HIGH_TEMP_SENSOR_READ_DELAY = 10000; // Delay between sensor readings in high temperature state
...
void setup() {
    ...
    Serial.println("============================");
    Serial.println("Device starting... Firmware v1.5");
    Serial.println("Mode: Enhanced Health Check with Temperature Management");
    Serial.println("============================");
...
void validateSensors() {
    ...
    if (lastTemperatureReading < 0) {
        ...
        if (lastTemperatureReading > TEMP_THRESHOLD) {
            Serial.println("ALERT: Temperature exceeds safe limits!");
            // Implement cooling mechanism or alert system here
        }
    }
